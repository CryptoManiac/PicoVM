...
function readAssembly(path, callback) {
    ...
    console.log('Loading assembly code from ' + assemblyPath + '...');
    ...
    function parseAssemblyData(data, callback) {
        ...
        if(data[peOffset + 0] != 0x0B || data[peOffset + 1] != 0x01)
            throw "Invalid PE standard magic";
        if(data[peOffset + 2] != 0x08)
            throw "Invalid PE standard lmajor";
        if(data[peOffset + 3] != 0x00)
            throw "Invalid PE standard lminor";
